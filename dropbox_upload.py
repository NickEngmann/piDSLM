...
This is an example app for API v2 with modular functions for testability.
"""
...
def list_folder(dbx, folder, subfolder):
    ...
    Args:
        dbx: Dropbox client instance
        folder: Dropbox folder name
        subfolder: Subfolder path within folder

    Returns:
        dict: Mapping of filenames to metadata, or empty dict on error
    """
...
def download(dbx, folder, subfolder, name):
    ...
    Returns:
        bytes: File contents, or None if download failed
    """
    path = '/%s/%s/%s' % (folder, subfolder.replace(os.path.sep, '/'), name)
    while '//' in path:
        path = path.replace('//', '/')
...
def stopwatch(message):
    ...
    Args:
        message: Description of operation to time
    """
    t0 = time.time()
    try:
        yield
